...
export const aggregation1 = async(gender, age) => {
  try {
    return await userDao.aggregation1(gender, age);
  } catch (error) {
...
export const getAllUsers = async (page, limit) => {
  try {
    return await userDao.getAllUsers(page, limit);
  } catch (error) {
    throw new Error(error);
  }
};
